...
const PokemonCard = ({ pokemonUrl }) => {
  ...
  return (
    <div
      className={`hover:-translate-y-2 h-[280px] aspect-square relative border-[4px] top-0 right-0 z-10 overflow-hidden ${
        squareTransition
          ? borderStyledPokemonByPokemonType[pokemonInfo?.types[0]] +
            " rounded-3xl"
          : "border-black rounded-full"
      } transition-all duration-500 bg-white dark:bg-special-dark-blue`}
    >
      <div
        className={`w-full h-1/2 relative ${
          pokeballTransition ? "top-[140px] bg-bug-bg" : "-top-10"
      ...
      <div
        ...
        } transition-all duration-700 dark:border-black`}
      ></div>
      <Link
          ...
          <h4 className="text-md font-normal text-white flex gap-2 justify-center items-center">
            <span
              className={`px-2 rounded-full ${
                bgUnderColorByType[pokemonInfo?.types[0]]
              }`}
            >
              {pokemonInfo?.types[0]}
            </span>
            {pokemonInfo?.types[1] && (
              <span
                className={`px-2 rounded-full ${
                  bgUnderColorByType[pokemonInfo?.types[1]]
                }`}
              >
                {pokemonInfo?.types[1]}
              </span>
