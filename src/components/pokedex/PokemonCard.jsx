...
const PokemonCard = ({ pokemonUrl }) => {
  ...
  return (
    <div className={`h-[280px] aspect-square relative border-[4px] top-0 right-0 z-10 overflow-hidden ${squareTransition ? borderStyledPokemonByPokemonType[pokemonInfo?.types[0]] + " rounded-3xl" : 'border-black rounded-full'} transition-all duration-500 bg-white`}>
      ...
      <Link
        ...
        <section className="font-Inter">
          ...
          <h4 className='text-lg font-bold'>
            <span className={`${textColorByType[pokemonInfo?.types[0]]}`}>{pokemonInfo?.types[0]}</span>
            {
              pokemonInfo?.types[1] && <span> / </span>
            }
            {
              pokemonInfo?.types[1] && <span className={`${textColorByType[pokemonInfo?.types[1]]}`}>{pokemonInfo?.types[1]}</span>
            }
          </h4>
